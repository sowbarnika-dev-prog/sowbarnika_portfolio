...
export const AboutSection = () => {
  ...
  const features = [
    ...
    "Performance-Optimized Cod",
    "Reusable React Component",
    "Team Leadership Experience (SME)",
    "User-Centric Development",
    "Reliable On-Time Deliver",
  ];

  const socialLinks = [
    {
      icon: <Github className="h-5 w-5" />,
      href: "https://github.com/sowbarnika-dev-prog",
    },
    {
      icon: <Linkedin className="h-5 w-5" />,
      href: "www.linkedin.com/in/sowbarnika-yoganathan-b4a97a237",
    },
  ...
  const tabContent = {
    personal:
      "Passionate about crafting beautiful and intuitive user interfaces that make an impact. When I'm not coding, I love exploring new frontend tools, improving my design sense, and experimenting with creative web animations.",

    professional:
      "With 2.5+ years of experience in frontend development, I've built 15+ responsive and high-performing websites using React, Tailwind, and modern JavaScript frameworks. I specialize in delivering pixel-perfect designs and seamless user experiences.",

    approach:
      "I believe great interfaces start with empathy for the user. My approach combines clean, reusable code, accessibility, and collaboration — ensuring every project is both visually stunning and technically sound.",
  };
  ...
  const handleDownload = () => {
    ...
    link.href = "/sowbarnika_portfolio/sowbarnika-resume.pdf"; // Must be in public folder
    link.download = "sowbarnika-resume.pdf";
            ...
            <div className="bg-card/50 border border-border rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:shadow-3xl hover:border-primary/40 hover:bg-card/60">
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 gap-4 sm:gap-6">
                {techStack.map((stack, index) => (
                  <div
                    key={index}
                    className="bg-background/50 border border-border rounded-2xl p-5 sm:p-6 transition-all duration-300 hover:border-primary/30 hover:scale-[1.02] hover:bg-background/60 group"
                  >
                    {/* Header */}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {stack.items.map((item, itemIndex) => (
                        <div
                          key={itemIndex}
                          className="flex items-center gap-3 text-sm sm:text-base text-muted-foreground hover:text-foreground transition-colors duration-300 border border-border/40 rounded-xl p-2 sm:p-3 hover:border-primary/40 hover:bg-background/70"
                        >
                          <div className="w-2 h-2 bg-primary rounded-full animate-pulse group-hover:scale-125 transition-transform" />
                          <span>{item}</span>
                        </div>
                      ))}
                    </div>
